...
def run_finance_calculations(params):
    ...
    # Sanity validation — catch impossible unit economics before computing
    ...
    if monthly_profit_at_scale <= 0:
        raise ValueError(
            f"Business not profitable at scale: monthly_profit_at_scale ({monthly_profit_at_scale}) must be positive."
        )
