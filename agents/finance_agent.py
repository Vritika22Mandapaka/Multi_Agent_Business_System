...
- If break_even_units_per_month is None: explain that the unit economics are broken (costs exceed revenue per unit)
...
BAD — "The ROI is 14.29%."
GOOD — "At 14.29% ROI, the business returns only $0.14 per dollar invested in year one, signaling that profitability depends heavily on scaling volume beyond year-one projections."

BAD — "Break-even is 17143 units per month."
GOOD — "Reaching break-even requires 17,143 orders per month — roughly 571 orders per day — which is an aggressive target for a college-focused niche and will require strong early marketing investment."

Apply this reasoning depth to every field."""
...
def finance_agent(state):
    ...
    # Stage 1: LLM extracts business-specific financial parameters
    params = _extract_financial_parameters(business_idea, research_output, tech_output)

    # Stage 2: Calculator computes metrics from the LLM-extracted parameters
    finance_calculations = run_finance_calculations(params)

    # Stage 2b: Compute numeric financial health signal to guide confidence scoring
    health_signal = _compute_financial_health_signal(finance_calculations)

    # Stage 3: LLM produces structured JSON analysis grounded in calculator results
...
{{
  ...
  "roi_projection": "What the year1_roi_percent means for investors — is it attractive or underwhelming for a first-year return, and why. Compare to industry norms where relevant.",
  "break_even_analysis": "What reaching break_even_units_per_month actually demands operationally — is it realistic. Reference comparable businesses where relevant.",
  "payback_period": "What payback_period_months signals about investment risk — is this fast or slow compared to similar startups in this sector",
  ...
  "confidence_reasoning": "2-3 sentence explanation of why this confidence score was assigned, referencing the specific numeric signals that drove it up or down"
}}"""
