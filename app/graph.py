...
def confidence_router(state):
    ...
    finance_analysis = finance_output.get("analysis", {})
    finance_score = finance_analysis.get("confidence_score") if isinstance(finance_analysis, dict) else None

    if isinstance(finance_score, int) and finance_score < 50:
        return "retry_agent"

    finance_text = str(finance_output).lower()
    low_signals = ["low confidence", "not feasible", "high risk", "negative roi", "broken unit economics"]
